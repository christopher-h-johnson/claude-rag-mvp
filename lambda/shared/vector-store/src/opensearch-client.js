...
export class OpenSearchVectorStore {
     ...
     */
    constructor(endpoint, indexName = 'documents', region) {
     ...
     * Requirements: 6.3
     */
    async indexEmbedding(embedding) {
        try {
     ...
     * Requirements: 6.3
     */
    async batchIndexEmbeddings(embeddings) {
        ...
        try {
            ...
            if (response.body.errors) {
                const erroredDocuments = response.body.items.filter((item) => item.index?.error);
     ...
     * Requirements: 7.2
     */
    async searchSimilar(queryVector, k, filters) {
        try {
