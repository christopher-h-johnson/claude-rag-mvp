...
describe('OpenSearchVectorStore', () => {
    ...
    describe('indexEmbedding', () => {
        ...
        it('should throw error when indexing fails', async () => {
            ...
            await expect(vectorStore.indexEmbedding(embedding)).rejects.toThrow('Failed to index embedding chunk-1');
        });
    });
    describe('batchIndexEmbeddings', () => {
        ...
        it('should throw error when bulk indexing has errors', async () => {
            ...
            await expect(vectorStore.batchIndexEmbeddings(embeddings)).rejects.toThrow('Bulk indexing failed for 1 documents');
        });
    ...
    describe('searchSimilar', () => {
        ...
        it('should throw error for invalid vector dimensions', async () => {
            ...
            await expect(vectorStore.searchSimilar(invalidVector, 5)).rejects.toThrow('Invalid query vector dimension: expected 1536, got 512');
        });
    ...
    describe('deleteDocument', () => {
        ...
        it('should throw error when deletion fails', async () => {
            ...
            await expect(vectorStore.deleteDocument('doc-1')).rejects.toThrow('Failed to delete document doc-1');
        });
    });
    describe('constructor', () => {
        it('should use custom index name', () => {
            const customStore = new OpenSearchVectorStore('test-endpoint.us-east-1.es.amazonaws.com', 'custom-index');
            expect(customStore.indexName).toBe('custom-index');
        });
        it('should use default index name', () => {
            expect(vectorStore.indexName).toBe('documents');
        });
        it('should initialize with correct endpoint format', () => {
            // Verify the client was created (implicitly tested by other tests)
            expect(vectorStore.client).toBeDefined();
            expect(vectorStore.indexName).toBe('documents');
        });
        it('should use custom region when provided', () => {
            const customStore = new OpenSearchVectorStore('test-endpoint.eu-west-1.es.amazonaws.com', 'documents', 'eu-west-1');
            expect(customStore.client).toBeDefined();
        });
    });
    describe('batchIndexEmbeddings - bulk API error handling', () => {
        it('should throw error when bulk API call fails completely', async () => {
            ...
            await expect(vectorStore.batchIndexEmbeddings(embeddings)).rejects.toThrow('Failed to batch index embeddings');
        });
    });
    describe('searchSimilar - advanced filtering', () => {
        ...
        it('should handle search errors gracefully', async () => {
            ...
            await expect(vectorStore.searchSimilar(queryVector, 5)).rejects.toThrow('Failed to search similar vectors');
        });
...
//# sourceMappingURL=opensearch-client.test.js.map