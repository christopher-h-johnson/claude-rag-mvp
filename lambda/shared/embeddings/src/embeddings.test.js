...
describe('EmbeddingGenerator', () => {
    ...
    describe('parallelBatchGenerateEmbeddings - Parallel Batch Processing', () => {
        it('should process texts in parallel batches', async () => {
            ...
            // Act
            const result = await generator.parallelBatchGenerateEmbeddings(texts, batchSize, concurrentBatches);
            // Assert
        ...
        it('should throw error for invalid concurrent batches parameter', async () => {
            ...
            const texts = ['Text 1', 'Text 2'];
            // Act & Assert
            await expect(generator.parallelBatchGenerateEmbeddings(texts, 25, 0)).rejects.toThrow('Concurrent batches must be at least 1');
            await expect(generator.parallelBatchGenerateEmbeddings(texts, 25, -1)).rejects.toThrow('Concurrent batches must be at least 1');
        });
        it('should call progress callback during parallel processing', async () => {
            ...
            // Act
            await generator.parallelBatchGenerateEmbeddings(texts, 5, 2, progressCallback);
            // Assert
