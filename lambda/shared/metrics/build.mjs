...
/**
 * Build script for shared metrics library
...
console.log('Building shared metrics library...\n');

// Step 1: Compile TypeScript
console.log('🔨 Compiling TypeScript...');
execSync('tsc', { stdio: 'inherit', cwd: __dirname });

// Step 2: Rename all .js files to .mjs and .d.ts to .d.mts in dist
console.log('\n📝 Renaming .js files to .mjs and .d.ts to .d.mts...');
...
if (existsSync(distDir)) {
    ...
    for (const file of files) {
        if (file.endsWith('.js')) {
            ...
            const oldPath = join(distDir, file);
            const newPath = join(distDir, file.replace('.d.ts', '.d.mts'));
            renameSync(oldPath, newPath);
            console.log(`  ✅ ${file} → ${file.replace('.d.ts', '.d.mts')}`);
        }
    }
}

// Step 3: Fix imports in .mjs and .d.mts files to reference .mjs instead of .js
console.log('\n🔧 Fixing import paths in .mjs and .d.mts files...');
...
console.log('\n✅ Build complete!');
console.log('📁 Output: dist/*.mjs and dist/*.d.mts\n');
